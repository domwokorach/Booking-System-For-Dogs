...
export class PaymentsService {
  ...
  async getSessionStatus(user: AuthUser, sessionId: string) {
    ...
    return {
      ...
      appointmentStatus: appointmentStatusResponse(
  ...
  async approveCancellation(token: string) {
    ...
    if (
      ...
      throw new BadRequestException(
        "This cancellation approval link is invalid, expired, or already used.",
      );
    }
    ...
    if (claimed.count !== 1) {
      throw new ConflictException(
        "This cancellation request is already being processed.",
      );
    }
  ...
  async adminApproveCancellation(appointmentId: string) {
    const staleProcessingBefore = new Date(
      Date.now() - APPROVAL_PROCESSING_TIMEOUT_MS,
    );
    const request = await this.findCancellationRequestForApproval({
  ...
  private findCancellationRequestForApproval(
    where: { tokenHash: string } | { appointmentId: string },
  ) {
  ...
  private async finalizeCancellationApproval(
    request: NonNullable<
      Awaited<
        ReturnType<PaymentsService["findCancellationRequestForApproval"]>
      >
    >,
  ) {
    try {
      const refund = await this.processApprovedRefund(
      ...
      if (
        ...
        if (session.payment_status !== "unpaid") {
          ...
          await this.fulfillPaidSession(session);
          const synchronizedRefund = await this.processApprovedRefund(
            request.appointmentId,
            request.id,
          );
          ...
          throw new ConflictException(
            "Stripe confirmed the payment, but Pawside could not prepare its refund. Please retry approval.",
          );
        }
        if (session.status === "complete") {
          throw new ConflictException(
            "Stripe is still processing this payment method. Pawside will update it when Stripe sends the success or failure webhook; retry approval afterwards.",
          );
        }
        if (session.status === "open") {
      ...
      await this.email.sendBookingCancellation({
        ...
        service:
          request.appointment.serviceRef?.name ?? request.appointment.service,
  ...
  private async processApprovedRefund(
    ...
    if (payment.status === PaymentStatus.RefundPending) {
      await this.finalizeApprovedCancellationRequest(
    ...
    return this.refundResponse(
      result.payment,
      "Cancellation approved. Your refund was submitted through Stripe and typically appears within 5–10 business days, depending on the bank.",
    );
  }

  private async finalizeApprovedCancellationRequest(
    cancellationRequestId: string,
    appointmentId: string,
  ) {
    const now = new Date();
...
function appointmentStatusResponse(status: AppointmentStatus): string {
  return status.replace(/([a-z])([A-Z])/g, "$1_$2").toUpperCase();
}
